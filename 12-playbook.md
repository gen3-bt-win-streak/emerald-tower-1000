...
→ **「速く殺す」以外に軽減手段が無い。** 3連続被弾は2.7%だが、試行を4回与えれば「どれか当たる」は76%。

#### 正着

1. **キングラー#358を見たら、サンダーの10まんボルトを最優先で叩き込む**（394-464 確1・S299>186で先手）
   - こらえられたら**次ターンも10まんボルト**（S299 > カムラ後279 で先手を維持できる）。こらえる2回目は50%で失敗する
2. **サンダーが場にいない／落ちているなら、メタグロスのだいばくはつを即座に切る**。
   温存則（§4.2「敵4体のうちは基本爆発しない」）**より優先**。この2セットは§8.1の危険分子と同格に扱う
3. **カイロス#369は発見即だいばくはつ**。確1が他に無く、カムラ後S442で先手も取れない。
   つるぎのまいを積まれる前に処理する
4. **ラティオスで削りにいかない**（246-290で1発足りず、2ターン分の試行を献上する）
