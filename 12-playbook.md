...
|---|---|---|---|---|
| **グロス** | **19** | 28 | 128 | オバヒ11＋だいもんじ5＋ガラガラ地震3 ＝ **全て種族名を見た瞬間に分かる**（炎タイプ＋ガラガラ）→ §1.6退避で消せる |
...
- ラグは被確2は94と**継戦耐久ならグロスより上**（H404）。柔らかいのではなく、**尾リスクの質が悪い**：
...
1. **控えを1体でも残しているうちは、敵ラス1の爆発は起きない**（−10で禁止）。自軍が2体になった瞬間に条項を切り替える
2. **メタグロスが場にいれば全滅しない**：**敵の爆発24セット全てをメタグロスは確定耐え**（最大被弾72%）
...
### 8.6b バンギラス全10セット（オープン限定・実戦で頻出）【2026-07-30 実測】

**結論：バンギラスはメタグロスを一度も確1できない。落ち着いて2発で処理する対面。**

| id | 持ち物 | 対グロス最大 | グロスのコメパン | 備考 |
|---|---|---|---|---|
| #860 / #861 | ひかりのこな | 47-56%（確定耐え） | **確2**（323-380 / HP341＝1発は69%、命中とこな込みで実効53%） | 回避10%で試行が増える |
| #868 | クイッククロー | 47-56%（確定耐え） | **確2**（同上） | QC20%で先手を取られ得る |
| #863 | クイッククロー | 52-62%（確定耐え） | 確2 | 特殊型（ボルトビーム）・旧解析での危険度3位 |
| #862 | きあいのハチマキ | 43-51%（確定耐え） | 確2 | ハチマキ10%で1耐えあり |
| #864 / #865 / #869 | カゴのみ / ラムのみ | 43-51%（確定耐え） | 確2 | |
| #866 | クイッククロー | 19-23% | 確2 | 低火力 |
| #867 | ラムのみ | 12-14% | 確2 | 最も安全 |

- **どのセットも最大ロールでグロスを落とせない**（最大62%）＝グロスは2発耐える。焦って爆発を切る必要はない。
- ただし**こちらも確1できない**（コメパン確2）。1発で落とせる確率は#860/#861/#868で69%、
  命中85%とひかりのこな×0.9を掛けると**実効53%**。**2ターンかかる前提で組み立てる**。
- ※旧A252型なら#860/#861/#868はコメパン**確1**だった（→ [15-v4-real-build.md](15-v4-real-build.md)「EV交換の実収支」）。
  現行v4では失われているので、**「バンギは1発」という古い感覚で動かないこと**。

### 8.8 「落ちそうで落ちない」水耐久・地震耐え【2026-07-30 実測・実戦報告を数値化】

> 実戦で「じしん＋10まんで落ちない」「デンリュウ/ブースターがじしんを耐える」と報告された対面を全数計算した。
> **原因は火力不足ではなく"撃ち手"**。タイプ一致の有無で同じ技が約1.4〜1.6倍変わる。

#### 大原則：**じしんはラグラージ、10まんはサンダー**

| 技 | タイプ一致あり | 一致なし | 倍率 |
|---|---|---|---|
| じしん | **ラグラージ**（水/地面・A350） | メタグロス（鋼/超・A381） | **ラグが約1.4倍**（攻撃はグロスが上なのに逆転） |
...
|---|---:|---|---|
| ラプラス#456 | 401 | 368-434 **乱数** | **405-477 確殺** |
| ラプラス#648 | 443 | 410-484 **乱数** | **453-535 確殺** |
| ラプラス#822 / #823 | 443 | 381-450 乱数 | **424-501 乱数（4%不足）** ←最硬 |
| ラプラス#552 / #744 / #820 / #821 | 401-464 | 488-575 確殺 | 541-638 確殺 |
| ミロカロス#655 | 373 | 347-409 **乱数** | **387-456 確殺** |
| ミロカロス#463 / #559 / #751 | 331-394 | 確殺 | 確殺 |

- **ラティオスの10まんを合算に使うと大半が届かない**（例：#456でグロス地震＋ラティ10万＝267-316／HP401）。
...
| 相手 | HP | グロス地震 | ラグ地震 |
|---|---:|---|---|
| デンリュウ#422 | 384 | 268-316 **耐え** | 372-438 乱1 |
| デンリュウ#710 | 321 | 221-260 **耐え** | 306-360 乱1 |
| デンリュウ#518 / #614 | 321 | 295-348 乱1 | **408-480 確1** |
| ブースター#540 | 313 | 253-298 **耐え** | **348-410 確1** |
| ブースター#444 | 271 | 251-296 乱1 | **346-408 確1** |
| ブースター#636 / #732 | 271 | 351-414 確1 | 484-570 確1 |
