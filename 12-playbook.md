...
## 8.9 開幕脅威トリオ（ルージュラ/レジアイス/プテラ）の統一開幕手順【2026-08-25・行動順込み改訂】

> 初版は「100%技コンボの合計値」だけで組んでいたが、走者指摘の通り**ターン内の行動順を無視していた**：
> **S338プテラ ＞ サンダー299 ＞ ルージュラ226 ＞ グロス177 ＞ レジアイス122-136**。
> ハチマキプテラ#531のげんしのちからはサンダーより先に飛び（乱1 56%）、ルージュラのれいB（#377/#665は確1）は
> サンダーの攻撃後・グロスの行動前に挟まる。行動順で組み直すと、3対面は**同じ開幕手順**に収束する。

### 統一手順：「T1 サンダーまもる ＋ グロス攻撃」から入る

この3種族が開幕に見えたら、**T1のサンダーは無条件でまもる**。1手の保険料で以下を全部遮断する：

| 遮断される即死 | 確率（撃ち合った場合） |
|---|---|
| ハチマキプテラ#531 げんしのちから | プテラ遭遇の23%×即死ロール56%＝**13%** |
| ルージュラ#377/#665 れいとうビーム | ルージュラの2/4セット・343-404＝**確1** |
| レジアイス#763/#774/#785/#839 れいB/ふぶき | 312-464＝乱1〜確1 |

### 対面別のT1グロスの技とT2以降

**プテラ**：T1 コメパン（85%・521-614全セット確1）。**T1の敵の行動順で型が割れる**——
サンダーより先に動けばS338（#435/#531）、後ならS296（#627/#723）。
- 外れ→T2：**S296型ならサンダー10まん423-498で100%確1**（先手が取れる）。
  S338型はサンダー→ラグ交代（#435受け28%/#531受け69%）＋コメパン再抽選。連続外れ2.25%
- ※EQは飛行無効・ラグめざ岩も確1不可＝S338型の1ターン解はコメパンのみ（構造的）

**ルージュラ（HP334・S226）**：T1 グロスEQ（命中100）。T2 **EQ＋10まん＝486-718で全セット100%確殺**。
コメパン不要。ルージュラの自由行動はまもる裏の1回だけで、あくまのキッスはラムで1回無効。
（サンダーで先に10まんを撃つ択は#377/#665のれいB確1が返るため禁止——走者指摘の通り）

**レジアイス**：T1 コメパン（85%・グロス177＞136で先手）。外れ→T2 コメパン再抽選＋サンダーはラティへ
（れいBはラティ137-162＝確3受け）。連続外れ2.25%。ハピ不在確認済みなら**爆発440-611が100%の即答**。

### 残存する命中依存の総括

| 対面 | 実効的な事故率 |
|---|---|
| プテラS296型 | **0%**（T2に10まん100%） |
| プテラS338型 | 2.25%（コメパン2連外し・その間ラグ受けで死なない） |
| ルージュラ | **0%**（EQ×2＋10まんの100%ルート） |
| レジアイス | 2.25%（同・ラティ受けで死なない） |

初版の「毎対面15%の即死露出」は、行動順を正しく織り込むと**「T1まもる固定＋2.25%の遅延リスク」**まで落ちる。
まもる1手のコストは、弱セット（#435・#475等）相手に1ターン損するだけで、マラソン期待値では圧倒的に黒字。
