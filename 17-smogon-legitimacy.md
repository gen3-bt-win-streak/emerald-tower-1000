...
PokeFinder＋独自全数探索で**実在確認済みのMethod-1スプレッド**をPKHeXで再現（実機capture無し）／イベントフラグ整合済み。

### 現行ルールでの掲載可能性：**高い**【原文ベースの評価】

1. 「合法的に入手可能なステータスと技」——全個体がPKHeXソースコード水準のMethod-1認定＋正規習得技のみ。**ルールが要求する水準を大幅に超過**。
...
- LICENSE（MIT）・英語README（再現手順つき）・.gitignore を付与して bundle 化
...
- [x] 4体すべて実在Method-1スプレッド（PKHeX Full ReportでMethod_1認定・16章§4）
