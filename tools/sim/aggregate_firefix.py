## 炎条項レバー(地震確定KO優先＋飛行炎10万)のA/B集計: FIRE_FIX ON(ffm) vs OFF=関所3ベースライン(v4m, 2516敗/100万)
## 同一100万シード(4ブロック×25万)のペア比較。両者はチーム/持ち物/EV/シード完全同一
## =探索の爆発候補採点を「相方まもる込み」にした修正の有無だけの差。
...
on,ton=load("ffm")    # FIRE_FIX=ON(修正)
off,toff=load("v4m")  # OFF=ベースライン(2516)

print("=== 炎条項レバー: FIRE_FIX ON(ffm) vs OFF=v4mベースライン 同一シード ===")
...
print("[FIRE_FIX ON] %d戦 負け%d 負け率%.4f%% Wilson95%%CI[%.4f%%,%.4f%%]"%(ton,tON,100*tON/max(1,ton),100*lo,100*hi))
