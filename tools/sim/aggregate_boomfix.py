## 爆発“まもる税”修正のA/B集計: BOOM_FIX ON(bfm) vs OFF=関所3ベースライン(v4m, 2516敗/100万)
...
on,ton=load("bfm")    # BOOM_FIX=ON(修正)
off,toff=load("v4m")  # OFF=ベースライン(2516)

print("=== 爆発まもる税修正: BOOM_FIX ON(bfm) vs OFF=v4mベースライン 同一シード ===")
...
print("[BOOM_FIX ON] %d戦 負け%d 負け率%.4f%% Wilson95%%CI[%.4f%%,%.4f%%]"%(ton,tON,100*tON/max(1,ton),100*lo,100*hi))
