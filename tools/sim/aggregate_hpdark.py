## ラグ技変種A/B: 冷B→めざ悪70 (HPDARK=hdm) vs OFF=v4mベースライン(2516敗/100万)
## 事前登録の合格基準(通常と異なる): 「有意に悪化しない」なら採用。
##   動機=実機の尾リスク潰し(ヌケニン即詰みルート: 岩雪崩実効85.5%→めざ悪95%の二段構え+ゲンガー打点改善)。
...
on,ton=load("hdm")    # HPDARK=ON(修正)
off,toff=load("v4m")  # OFF=ベースライン(2516)

print("=== 炎条項レバー: HPDARK ON(hdm) vs OFF=v4mベースライン 同一シード ===")
...
print("[HPDARK ON] %d戦 負け%d 負け率%.4f%% Wilson95%%CI[%.4f%%,%.4f%%]"%(ton,tON,100*tON/max(1,ton),100*lo,100*hi))
