...
GENGAR_RETREAT=True
def max_hit(b,f,m):
    # fair-info: 候補技ユニオン×pminmax(実セット覗き見なし)
    best=0
...
def winner_choose(b):
    ...
    if b.turn==1 and gross is not None and fire:
        ...
        if cands and not wobb:
            ...
            if gengar is not None:
                ga=acts.get(gengar)
                keep = ga and ga[0]=="move" and ga[1] in MOVES and MOVES[ga[1]]["power"]>=2 and ga[2] is not None and b.pminmax(gengar,ga[2],ga[1])[0]>=ga[2].hp
                if not keep:
    ...
    if gross is not None and gengar is not None and not fire:
        ...
        gengar_kills = ga_n and ga_n[0]=="move" and ga_n[1] in MOVES and MOVES[ga_n[1]]["power"]>=2 and ga_n[2] is not None and b.pminmax(gengar,ga_n[2],ga_n[1])[0]>=ga_n[2].hp
        if not gross_boom:
    ...
    if GENGAR_RETREAT and gengar is not None and b.turn>=2:
        ga=acts.get(gengar)
        atk_kills = ga and ga[0]=="move" and ga[1] in MOVES and MOVES[ga[1]]["power"]>=2 and ga[2] is not None and b.pminmax(gengar,ga[2],ga[1])[0]>=ga[2].hp
