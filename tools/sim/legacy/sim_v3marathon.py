...
if __name__=="__main__":
    BASE=int(sys.argv[1]); N=int(sys.argv[2]) if len(sys.argv)>2 else 250000
    CKPT="v3m_ckpt_%d.json"%BASE
    st=dict(i=0,losses=[],streak=0,best=0,run_start=BASE,s250=0,s500=0,s750=0,s1000=0)
    if os.path.exists(CKPT): st.update(json.load(open(CKPT)))
    t0=time.time(); i0=st["i"]
    def event(kind,seed,streak):
        open("milestones.jsonl","a").write(json.dumps(dict(w=BASE,kind=kind,run_start=st["run_start"],hit=seed,streak=streak))+"\n")
    while st["i"]<N:
        seed=BASE+st["i"]
        _battle_no[0]=st["i"]+1
        ...
        if r=="win":
            ...
            if s>st["best"]:
                st["best"]=s
                if s>=1458 : event("NEWBEST",seed,s)  # プロジェクト歴代最長1458の更新のみ即時
        else:
        ...
        if st["i"]%1000==0:
            ...
            print("[W%d] %d/%d 負け%d(%.3f%%) 連勝中%d 最長%d 250/500/750/1000=%d/%d/%d/%d %d戦/分"%(
                BASE,st["i"],N,len(st["losses"]),100*len(st["losses"])/st["i"],st["streak"],st["best"],
                st["s250"],st["s500"],st["s750"],st["s1000"],(st["i"]-i0)/max(1,(time.time()-t0)/60)),flush=True)
    json.dump(st,open(CKPT,"w"))
    print("[W%d] 完走 %d戦 負け%d(%.3f%%) 最長%d 1000到達%d回"%(BASE,N,len(st["losses"]),100*len(st["losses"])/N,st["best"],st["s1000"]),flush=True)
