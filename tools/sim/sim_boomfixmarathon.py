...
os.environ["BOOM_FIX"]="1"  # A/B ON側: 探索の爆発“まもる税”修正を有効化
...
if __name__=="__main__":
    ...
    CKPT="bfm_ckpt_%d_%d.json"%(BASE,START)
    ...
    def event(kind,seed,streak):
        open("milestones_bfm.jsonl","a").write(json.dumps(dict(w=BASE,start=START,kind=kind,run_start=st["run_start"],hit=seed,streak=streak))+"\n")
    while st["i"]<N:
