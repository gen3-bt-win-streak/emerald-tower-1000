        ...
        if fire is not None and icem is not None and not grass and z is not None and g2 is not None:
            if fair_maxhit(b,fire,g2)>=g2.hp and fair_maxhit(b,icem,z)>=z.hp:
                if b.turn==1:
                    ...
                    if recv is not None: acts[g2]=("switch",g2,recv)
                    if "MOVE_PROTECT" in z.moves and z.protect_streak<1:
                        acts[z]=("move","MOVE_PROTECT",z)
                    clause=True
                else:
...
if __name__=="__main__":
    ...
    CKPT="%s_off_ckpt_%d.json"%(TAG,BASE)
    st=dict(i=0, losses=[])
    ...
    while st["i"]<N:
        ...
        if r!="win": st["losses"].append(seed)
        st["i"]+=1
        if st["i"]%1000==0:
            json.dump(st,open(CKPT,"w"))
            print("[%s@%d] %d/%d 負け%d (%.3f%%) %d戦/分"%(TAG,BASE,st["i"],N,len(st["losses"]),100*len(st["losses"])/st["i"],(st["i"]-i0)/max(1,(time.time()-t0)/60)),flush=True)
    json.dump(st,open(CKPT,"w"))
    print("[%s@%d] 負け率 %.3f%% (%d/%d)"%(TAG,BASE,100*len(st["losses"])/N,len(st["losses"]),N),flush=True)
    print("負けシード:",st["losses"],flush=True)
