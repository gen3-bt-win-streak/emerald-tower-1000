...
os.environ["FIRE_FIX"]="1"; os.environ["HP_ROCK"]="1"  # A/B ON側: めざ岩レバー。OFF側=ffm(FIRE_FIXのみ)と同一シードでHP_ROCKだけの差を分離
...
# めざ岩70注入(D案・2026-07-30の実機チーム変更に追随。物理いわ・命中100・単体・追加効果なし)
# HP_ROCK=1で有効化(既定OFF=凍結マラソンと挙動一致): ラグ=いわなだれ→めざ岩70 / EV B4→S4 /
# 実在Method-1個体 E6BA7F73 (31/31/30/31/22/30) の実数値 404/350/215/185/207/156 へ補正
HP_ROCK = os.environ.get("HP_ROCK")=="1"
if HP_ROCK:
...
def spec_v3():
    ...
    if HP_ROCK:
        ...
        w[2]={"hp":252,"atk":252,"spe":4}                           # B4→S4(めざ岩個体はS-IV30のためEV4でS156死守)
        s[3]=tuple(w)
    return s
...
# 追加レバー(env-gated・既定OFF=byte一致)。単炎条項Cのみ:
        ...
        if z is not None and g2 is not None and len(fires)==1:
            ...
            if danger and z_safe:
                if b.turn==1 and fire.status is None:
                    # FIRE_FIX-②: 飛行炎(地震無効)はサンダー10万で確定KOできるならT1で処理(麻痺不要・メタ温存)
                    if FIRE_FIX and "TYPE_FLYING" in fire.types and "MOVE_THUNDERBOLT" in z.moves \
                       and b.pminmax(z,fire,"MOVE_THUNDERBOLT")[0]>=fire.hp:
                        acts[z]=("move","MOVE_THUNDERBOLT",fire)
                    else:
                    ...
                    # FIRE_FIX-①: 麻痺炎を地震で確定KOできるなら爆発でなく地震(メタグロス温存)
                    if FIRE_FIX and "MOVE_EARTHQUAKE" in g2.moves \
                       and b.pminmax(g2,fire,"MOVE_EARTHQUAKE")[0]>=fire.hp:
                        acts[g2]=("move","MOVE_EARTHQUAKE",fire)
                    else:
...
if __name__=="__main__":
    ...
    CKPT="hrm_ckpt_%d_%d.json"%(BASE,START)
    ...
    def event(kind,seed,streak):
        open("milestones_hrm.jsonl","a").write(json.dumps(dict(w=BASE,start=START,kind=kind,run_start=st["run_start"],hit=seed,streak=streak))+"\n")
    while st["i"]<N:
