...
MOVES["MOVE_HP_DARK"]=dict(MOVES["MOVE_CRUNCH"], power=70, effect="EFFECT_HIT")  # ラグALL31のめざパ=あく70(A/B変種)
...
def spec_v3():
    ...
    s[1]=tuple(g)
    w=list(s[3]); wm=list(w[4]); wm[1]="MOVE_HP_DARK"; w[4]=wm; s[3]=tuple(w)  # ラグ冷B→めざ悪70(A/B変種)
    return s
...
if __name__=="__main__":
    ...
    CKPT="hdm_ckpt_%d_%d.json"%(BASE,START)
    ...
    def event(kind,seed,streak):
        open("milestones_hdm.jsonl","a").write(json.dumps(dict(w=BASE,start=START,kind=kind,run_start=st["run_start"],hit=seed,streak=streak))+"\n")
    while st["i"]<N:
