...
if __name__=="__main__":
    ...
    CKPT="v5m_ckpt_%d_%d.json"%(BASE,START)
    ...
    def event(kind,seed,streak):
        open("milestones_v5.jsonl","a").write(json.dumps(dict(w=BASE,start=START,kind=kind,run_start=st["run_start"],hit=seed,streak=streak))+"\n")
    while st["i"]<N:
