## v0.8の情報パリティ修正版: max_hitを候補集合ベースに差し替えて再測定
## argv: seedbase (default 400000)
...
## ---- 情報パリティ修正: 実セット覗き見のmax_hitを候補集合ベースへ ----
cand_moves=G["cand_moves"]
def max_hit(b,f,m):
    best=0
    for fm in (cand_moves(f) if f.side=="foe" else f.moves):
        ...
        best=max(best,hi)
    return best
## winner_choose / 各armはグローバルのmax_hitを参照するため、この再定義で全経路が公平化される
...
SUBMOVES=["MOVE_THUNDERBOLT","MOVE_PSYCHIC","MOVE_SUBSTITUTE","MOVE_PROTECT"]
...
def choose(b):
    ...
    pair_finish(b,acts)
    gengar_sub_early(b,acts)
    return acts
def bld():
    spec=list(W)
    spec[0]=("Gengar","Timid",{"hp":224,"spa":32,"spe":252},"Bright Powder",SUBMOVES,"ABILITY_LEVITATE","M")
    team=[]
    ...
    return team
BASE=int(sys.argv[1]) if len(sys.argv)>1 else 400000
N=20000
G["our_team"]=bld; G["our_choose"]=choose
G["POLICY_VARIANT"]="G"; G["PROTECT_CAP"]=1
res=collections.Counter()
for i in range(N):
    b,r=G["play_battle"](seed=BASE+i)
    res[r]+=1
print("[v0.8公平版@%d] 負け率 %.3f%% (%d/%d)"%(BASE,100*res["loss"]/N,res["loss"],N),flush=True)
