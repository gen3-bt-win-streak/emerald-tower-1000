...
4. **`AI_CV_SelfKO` の `if_target_faster` はダブルで対象ごとに評価されるか**（→[20](20-explosion-digest.md) §3.5③）。
   対象別なら「こちらに敵より速い駒が1体でも場にいると、満タン抑制の -3 が外れて爆発が飛ぶ」という結論になる。
   条項の存在は原文確定、**対象別評価は採点ループ構造からの推定で逐条確認が未了**（要：`data/battle_ai_scripts.s` の該当ラベルとターゲット参照の読解）
...
6. **みやぶる／かぎわけるの実効命中**（→[22](../shelved/22-s-axis.md) §8副産物1・[18](../current/18-verification-ledger.md) 2026-08-29）。
   ...
   `accuracycheck` → `setforesight` の順に並ぶため机上では「1発目は回避ランクを受ける（+6で33%）」と読める。
   しかし**ユーザーの実機運用では、かげぶんしんを積んだ相手に対してみやぶるが外れた事例が一度もない**。
   机上の読みと実測が食い違っており、**実機で試行回数を取るのが唯一の決着手段**。
   これが「必中」なら、かげぶんしん56セット（10.3%）への恒久解除ボタンとしての価値が大きく上がる
